...
from monitoring.utils import aesEncrypt, aesDecrypt, intPackBytes
from monitoring.error import DecodeError
...
class MACPayload(object):
    ...
    def decode(cls, data):
        ...
        frmpayload = None
        if dlen > fhdr.length:
            fport = struct.unpack('B', data[fhdr.length:fhdr.length+1])[0]
        # Decode frmpayload
